...
import {
  ...
  RotateCcw, Phone, CreditCard, ExternalLink, Download } from
"lucide-react";
            ...
            <div key={p._id} className="p-6 bg-[#111827]/80 border border-white/10 rounded-3xl group hover:border-yellow-500/30 transition-all flex items-center justify-between shadow-xl">
                                <div className="flex items-center gap-5">
                                    <div className="w-14 h-14 bg-white/5 rounded-2xl overflow-hidden border border-white/10 shrink-0 grayscale opacity-40">
                                        <img src={getMediaUrl(p.imageUrl || p.photo?.s3 || p.photo?.drive, '/placeholder-player.png')} className="w-full h-full object-cover" alt="" />
                                    </div>
                                    <div>
                ...
                <div key={p._id} className="p-6 bg-[#111827]/80 border border-white/10 rounded-3xl group hover:border-violet-500/30 transition-all flex items-center justify-between shadow-xl">
                                  <div className="flex items-center gap-5">
                                     <div className="w-14 h-14 bg-white/5 rounded-2xl overflow-hidden border border-white/10 shrink-0">
                                        <img src={getMediaUrl(p.imageUrl || p.photo?.s3 || p.photo?.drive, '/placeholder-player.png')} className="w-full h-full object-cover group-hover:scale-110 transition-transform" alt="" />
                                     </div>
                                     <div>
                  ...
                  className="w-56 h-56 object-cover rounded-[2.5rem] border-4 border-white/10 shadow-xl" alt="" />
                      ...
                      <div className="w-full space-y-6">
                         <div className="p-5 bg-white/5 rounded-2xl border border-white/10 group cursor-pointer hover:bg-white/10 transition-all" onClick={() => window.open(selectedPlayer.aadhaarUrl, '_blank')}>
                            ...
                            {selectedPlayer.aadhaarUrl ? <img src={selectedPlayer.aadhaarUrl} className="w-full h-32 object-cover rounded-xl" alt="" /> : <div className="h-32 rounded-xl bg-slate-900 flex items-center justify-center text-[9px] font-black text-slate-800 uppercase italic">No identity proof attached</div>}
                         </div>
                      </div>
                   </div>
