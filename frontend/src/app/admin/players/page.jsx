...
function PlayersRegistryContent() {
  ...
  const [whatsAppPlayer, setWhatsAppPlayer] = useState(null);
  ...
  const handleSendWhatsApp = () => {
    if (!whatsAppPlayer || !whatsAppMessage) return;
    const url = `https://wa.me/91${whatsAppPlayer.mobile}?text=${encodeURIComponent(whatsAppMessage)}`;
    window.open(url, "_blank");
    setWhatsAppModalOpen(false);
    setWhatsAppPlayer(null);
    setWhatsAppMessage("");
  };
  ...
  const downloadPlayersPDF = async () => {
    ...
    try {
      ...
      doc.save(`${safeTournamentName}_${fileSuffix}_Registry.pdf`);
    } catch (err) {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-20">
      ...
      {whatsAppModalOpen && whatsAppPlayer && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-slate-950/85 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="relative w-full max-w-lg bg-slate-900 border border-white/10 rounded-3xl p-8 shadow-2xl animate-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between mb-6 pb-4 border-b border-white/5">
              <div>
                <p className="text-[9px] font-black text-emerald-400 uppercase tracking-[0.4em] mb-1">WhatsApp Dispatch</p>
                <h3 className="text-xl font-black text-white uppercase tracking-wider">Edit Confirmation Message</h3>
              </div>
              <button 
                onClick={() => { setWhatsAppModalOpen(false); setWhatsAppPlayer(null); }} 
                className="p-2 hover:bg-white/5 rounded-full text-slate-400 hover:text-white transition-all border border-white/5"
              >
                <X className="w-5 h-5" />
              </button>
            ...
            <div className="space-y-4 mb-6">
              <div>
                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 block">Recipient Player</label>
                <div className="p-3 bg-white/5 rounded-xl border border-white/5 flex items-center justify-between">
                  <span className="text-sm font-bold text-white">{whatsAppPlayer.name}</span>
                  <span className="text-xs text-slate-400">ID: #{whatsAppPlayer.applicationId} | Mobile: {whatsAppPlayer.mobile}</span>
                </div>
              </div>
              <div>
                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 block">Message Content</label>
                <textarea
              ...
              <p className="text-[9px] text-slate-500 leading-relaxed italic">
                Note: You can use WhatsApp markdown like *bold* or _italics_. Clicking send will open WhatsApp Web or the WhatsApp Desktop App automatically.
              </p>
            </div>
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 py-4 bg-emerald-600 hover:bg-emerald-500 rounded-2xl text-[10px] font-black uppercase tracking-widest text-white shadow-xl shadow-emerald-500/20 transition-all flex items-center justify-center gap-2"
              >
